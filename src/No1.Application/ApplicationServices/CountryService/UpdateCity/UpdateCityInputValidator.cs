...
public class UpdateCityInputValidator : AbstractValidator<UpdateCityInput>
{
    private readonly ICountryRepository _countryRepository;
    private readonly IStringLocalizer<No1Resource> _l;

    public UpdateCityInputValidator(ICountryRepository countryRepository, IStringLocalizer<No1Resource> l)
    {
    ...
    private async Task<bool> NotAlreadyExistCity(UpdateCityInput input, CancellationToken cancellationToken)
    {
        ...
        var exitingCity = country.Cities.FirstOrDefault(city => city.Name == input.Name && city.Id != input.CityId);

        return exitingCity is null;
    }
}